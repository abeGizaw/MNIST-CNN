...
class StatisticsTracker:
    def __init__(self, model_name, stats_file):
        ...
        # Add a data directory if it does not already exist
    ...
    def load_statistics(self):
        if os.path.exists(self.stats_file):
            with open(self.stats_file, 'r') as file:
                ...
                #get(x, y) returns new value if times ran found. Else retains y
    ...
    def save_statistics_to_json(self):
        ...
        # Update the epoch accuracies with new data. Loop over each (key, value) pair
    ...
    def record_epoch_accuracy(self, epoch, accuracy):
        strEpoch = str(epoch)
        if strEpoch not in self.epoch_accuracies:
            self.epoch_accuracies[strEpoch] = []
        self.epoch_accuracies[strEpoch].append(accuracy)
